...
function kernelExploit() {
 ...
 var trigger_spray = function () {
  ...
  if(espMode == "1" || espMode == "2"){
   var timeleft = waitForUSB;
   cacheUPDtxt.innerHTML = "Menjalankan Kernel Exploit, Harap Tunggu ... "+timeleft/1000;
   var downloadTimer = setInterval(function(){
    if(timeleft <= 1000){
	 cacheUPDtxt.innerHTML="Menjalankan Kernel Exploit, Harap Tunggu ... <span style='font-family:monospace;opacity:0;'>0.0</span>";
     ...
     setTimeout(function(){
      ...
      cacheUPDtxt.innerHTML="Eksploitasi Kernel Gagal! Nyalakan Ulang dan Coba Lagi.<br>Tekan X Jika Anda Perlu Mengubah Pengaturan ESP.";
      alert("Eksploitasi Kernel Gagal! Nyalakan Ulang dan Coba Lagi."); 
     }
    },100);	
   }
   else{disptime=timeleft/1000;cacheUPDtxt.innerHTML="Menjalankan Kernel Exploit, Harap Tunggu ... <span style='font-family:monospace'>"+disptime.toFixed(1)+"</span>";}
   timeleft -= 100;
   },100);
  }
  else {
 ...
 //     sleep(3000);
      ...
      failCounter0.innerHTML=localStorage.failcount;
      cacheUPDtxt.innerHTML="Eksploitasi Kernel Gagal! Nyalakan Ulang dan Coba Lagi.";
      alert("Eksploitasi Kernel Gagal! Nyalakan Ulang dan Coba Lagi."); 
     } 
   resolve();
   }, {once: true});
...
function run_hax() {
 ...
 if (chain.syscall(23, 0).low != 0x0) {
  if(espMode == "1" || espMode == "2")enableUSB();
  localStorage.failcount= ++localStorage.failcount;
  localStorage.tempFailcount= ++localStorage.tempFailcount;
  localStorage.kexStatus = "started"; 
  kernelExploit();
 }
 else if (chain.syscall(23, 0).low == 0) {
